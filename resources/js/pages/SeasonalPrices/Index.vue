...
const submit = () => {
    if (editingPrice.value) {
        form.put(route('properties.seasonal-prices.update', [props.property.id, editingPrice.value.id]), {
            onSuccess: () => isDialogOpen.value = false,
        });
    } else {
        form.post(route('properties.seasonal-prices.store', props.property.id), {
            onSuccess: () => isDialogOpen.value = false,
        });
    }
};
...
const deletePrice = (price: SeasonalPrice) => {
    if (confirm('Are you sure you want to delete this seasonal price?')) {
        router.delete(route('properties.seasonal-prices.destroy', [props.property.id, price.id]));
    }
};
...
const breadcrumbs = [
    ...
    },
    {
        ...
        href: `/properties/${props.property.id}/seasonal-prices`,
    },
];
</script>
